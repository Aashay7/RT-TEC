...
CANARY_ENABLED = os.getenv("CANARY_ENABLED","false").lower() in ("1","true","yes")
...
FEAT = Histogram("feature_latency_ms", "Feature latency (ms)", buckets=(1,2,3,5,8,13,21,34))
POL = Histogram("policy_latency_ms", "Policy latency (ms)", buckets=(1,2,3,5,8,13,21,34))
...
def _startup():
    ...
    try:
        ...
        with httpx.Client(timeout=httpx.Timeout(connect=0.3, read=2.0, write=0.5, pool=0.5)) as client:
            r = client.post(TRITON_INFER, json=infer_req); r.raise_for_status()
...
async def score(payload: FeatureVector, request: Request):
    ...
    try:
        ...
        vec = vals[0] if (isinstance(vals, list) and vals and isinstance(vals[0], list)) else vals
        ...
        try:
            async with httpx.AsyncClient(timeout=httpx.Timeout(connect=0.3, read=2.0, write=0.5, pool=0.5)) as client:
                ...
                if cr.status_code == 200:
                    ...
                    cvec = cvals[0] if (isinstance(cvals, list) and cvals and isinstance(cvals[0], list)) else cvals
                    canary_prob = float(cvec[1])
                    from math import fabs
                    CANARY_DELTA.observe(abs(prob_trade - canary_prob))
                    # decision disagreement?
                    from .guardrails import decide as dec2
                    cdec, _, _ = dec2(spread_bps=spread_bps, prob_trade=canary_prob)
                    if cdec != decision:
                        CANARY_DISAGREE.inc()
                else:
                    pass
    ...
    decision, conf, reason = decide(spread_bps=spread_bps, prob_trade=prob_trade)
...
async def debug_features(symbol: str = Query(..., pattern=r"^[A-Z.]{1,15}$")):
    ...
    try:
        r = _redis.from_url(redis_url, socket_connect_timeout=0.2, socket_timeout=0.5)
        redis_ok = (r.ping() is True)
        redis_error = None
    except Exception as e:
        redis_ok = False; redis_error = str(e)
...
CANARY_DELTA = Histogram("canary_abs_delta", "Abs difference between primary and canary prob", buckets=(0.0,0.01,0.02,0.05,0.1,0.2,0.5,1.0))
CANARY_DISAGREE = Counter("canary_disagree_total", "Count of decision disagreement between primary and canary")
